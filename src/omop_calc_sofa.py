...
def score_respiratory(pfratio, on_vent):
    """SOFA respiratory 0-4. Vincent 1996 thresholds."""
    if pd.isna(pfratio):
        return np.nan
    if pfratio > 400:
        return 0
...
def score_cardiovascular(map_val, vaso_rates):
    """
    vaso_rates: dict with keys norepi, epi, dopamine, dobutamine (mcg/kg/min)
    """
    ...
    if dobu > 0 and total_norepi_eq == 0 and dopa == 0:
        return 2

    if dopa > 0 and dopa <= 5 and total_norepi_eq == 0:
        return 2

    if dopa > 15 or total_norepi_eq > 0.1:
        return 4

    if dopa > 5 or total_norepi_eq <= 0.1:
        return 3
...
def compute_daily_sofa(cdm, ancestor_df=None, visit_start_end=None, min_components=1, impute_missing_as_zero=False):
    """
    ...
    - min_components: minimum non-missing organ scores to compute total (default 1)
    - impute_missing_as_zero: if True, treat missing organs as 0 (normal)

    Returns:
    - DataFrame with person_id, visit_occurrence_id, chartdate, total_sofa, components_present, and 6 components
    """
    ...
    grid = merge_locf(grid, vent, 'on_vent', '24H')
    grid['on_vent'] = grid['on_vent'].fillna(0).astype(int)
    grid = merge_locf(grid, rrt, 'on_rrt', '24H')
    ...
    print(f"Days with <{min_components} components: {(daily['components_present'] < min_components).sum()}")
