...
def compute_hourly_sofa(cdm, ancestor_df=None, impute_missing_as_zero=True):
    ...
    def merge_locf(grid, ts, value_col, window='4h'):
        ...
        merged = pd.merge_asof(grid.sort_values('charttime'), ts, by=['person_id','visit_occurrence_id'], on='charttime', direction='backward', tolerance=pd.Timedelta(window))
        grid[value_col] = merged[value_col]
        return grid
    grid = merge_locf(grid, map_ts, 'map', '8h')
    ...
    if impute_missing_as_zero:
        grid_filled = grid[comp_cols].fillna(0)
        grid['total_sofa'] = grid_filled.sum(axis=1)
    else:
        grid['total_sofa'] = grid[comp_cols].sum(axis=1, min_count=1)
    grid['components_present'] = grid[comp_cols].notna().sum(axis=1)
    print(f"Generated hourly SOFA grid: {len(grid)} rows")
    return grid[['person_id','visit_occurrence_id','charttime','total_sofa','components_present'] + comp_cols]

def compute_daily_sofa(cdm, ancestor_df=None, min_components=1, impute_missing_as_zero=True):
    hourly = compute_hourly_sofa(cdm, ancestor_df, impute_missing_as_zero)
    hourly['chartdate'] = hourly['charttime'].dt.floor('D')
    daily = hourly.groupby(['person_id','visit_occurrence_id','chartdate']).agg({'total_sofa':'max','components_present':'max','resp_sofa':'max','cardio_sofa':'max','neuro_sofa':'max','hepatic_sofa':'max','renal_sofa':'max','coag_sofa':'max'}).reset_index()
    return daily
