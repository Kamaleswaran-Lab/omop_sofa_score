...
CONCEPT_SEEDS = {
    ...
    'vasopressin': [11149],
    'antibiotics_systemic': [1738622, 1713332, 1717327, 1707164, 1742537, 1750239, 1771205],
    'culture_procedure': [4046279, 4149581, 4162370],
    ...
    if VERBOSE:
        ...
        if data is not None:
            if isinstance(data, pd.DataFrame):
                ...
                print(f"Shape: {data.shape}, dtypes: {dict(data.dtypes)}")
            else:
                print(data)
        ...
        for standard_name, df in cdm.items():
            ...
            if standard_name == 'drug_exposure':
                if 'dose_unit_concept_id' not in df.columns:
                    ...
                    vprint(f"Added missing dose_unit_concept_id to drug_exposure")
                ...
                if 'quantity' in df.columns and 'sig' in df.columns:
                    ...
                    after_numeric = df['quantity'].notna().sum()
                    ...
                    after_extract = df['quantity'].notna().sum()
                    
                    if VERBOSE and (after_extract > before):
                        vprint(f"drug_exposure quantity rescue: {before} -> {after_numeric} (numeric SIG) -> {after_extract} (extracted)")
            ...
            for col in df.columns:
                if 'date' in col or 'time' in col:
                    ...
                    if VERBOSE and before_nonnull != after_nonnull:
                        vprint(f"{standard_name}.{col}: parsed dates, lost {before_nonnull - after_nonnull} values")
            
            for id_col in ['person_id', 'visit_occurrence_id']:
                if id_col in df.columns:
                    df[id_col] = pd.to_numeric(df[id_col], errors='coerce').astype('Int64')
            
            vprint(f"Normalized {standard_name}: {original_shape} -> {df.shape}")
...
def expand_concepts(ancestor_df, seed_ids):
    try:
        if ancestor_df is None or ancestor_df.empty:
            vprint(f"expand_concepts: no ancestor_df, using seeds only")
        ...
        vprint(f"expand_concepts: seeds={len(seed_ids)}, descendants={len(descendants)}, total={len(result)}")
        return result
    except Exception as e:
        print(f"ERROR in expand_concepts: {e}")
        if VERBOSE:
            traceback.print_exc()
        return list(set(seed_ids))
...
def get_measurements(cdm, seed_keys, domain=None, ancestor_df=None):
    try:
        ...
        vprint(f"get_measurements {seed_keys}: found {len(df)} rows for {len(concept_ids)} concepts")
        ...
        if VERBOSE and before != after:
            vprint(f"get_measurements {seed_keys}: dropped {before-after} rows with null value/time")
        
        if domain:
            df = convert_units(df, 'value', 'unit_concept_id', domain)
        
        result = df[['person_id','visit_occurrence_id','charttime','value','unit_concept_id']]
        vprint(f"get_measurements {seed_keys}", result)
        return result
    except Exception as e:
        print(f"ERROR in get_measurements {seed_keys}: {e}")
        if VERBOSE:
            traceback.print_exc()
        return pd.DataFrame(columns=['person_id','visit_occurrence_id','charttime','value','unit_concept_id'])
...
def derive_map(cdm, ancestor_df=None):
    try:
        ...
        vprint(f"derive_map: direct={len(direct)}, sbp={len(sbp)}, dbp={len(dbp)}")
        
        if sbp.empty or dbp.empty:
            result = direct[['person_id','visit_occurrence_id','charttime','value','source']] if not direct.empty else pd.DataFrame(columns=['person_id','visit_occurrence_id','charttime','value','source'])
            vprint(f"derive_map returning direct only", result)
            return result
        ...
        result = pd.concat([direct[['person_id','visit_occurrence_id','charttime','value','source']], derived], ignore_index=True)
        vprint(f"derive_map: derived {len(derived)} MAP from SBP/DBP", result)
        return result
    except Exception as e:
        print(f"ERROR in derive_map: {e}")
        if VERBOSE:
            traceback.print_exc()
...
def derive_gcs(cdm, ancestor_df=None):
    try:
        ...
        vprint(f"derive_gcs: found {len(total)} GCS measurements")
        if total.empty:
            return pd.DataFrame(columns=['person_id','visit_occurrence_id','charttime','value','source'])
        total['source'] = 'total'
...
def get_paired_pao2_fio2(cdm, ancestor_df=None):
    try:
        ...
        vprint(f"get_paired_pao2_fio2: pao2={len(pao2)}, fio2={len(fio2)}")
        ...
        if fio2.empty:
            ...
            vprint(f"get_paired_pao2_fio2: no FiO2, assuming room air", pao2)
            return pao2
        ...
        vprint(f"get_paired_pao2_fio2: paired {before}->{after} measurements", paired)
...
def get_urine_output_24h(cdm, ancestor_df=None):
    try:
        ...
        vprint(f"get_urine_output_24h: found {len(uo)} measurements")
        if uo.empty:
        ...
        result = daily.rename(columns={'value':'uo_24h_ml'})
        vprint(f"get_urine_output_24h: aggregated to {len(result)} days", result)
        return result
    except Exception as e:
        print(f"ERROR in get_urine_output_24h: {e}")
        if VERBOSE:
            traceback.print_exc()
        return pd.DataFrame(columns=['person_id','visit_occurrence_id','chartdate','uo_24h_ml'])
...
def get_vasopressors(cdm, ancestor_df=None):
    try:
        ...
        vprint(f"get_vasopressors: found {len(v)} records for {len(drug_ids)} drug concepts")
        ...
        if (~has_qty).any():
            ...
            if VERBOSE:
                print("--- First 5 problematic vasopressor records ---")
                cols = ['person_id','drug_concept_id','start','end','quantity','sig','duration_hr','weight_kg']
                cols = [c for c in cols if c in missing.columns]
                print(missing[cols].head(5).to_string(index=False))
                if 'sig' in missing.columns:
                    print("\n--- Unique SIG examples ---")
                    sig_examples = missing['sig'].dropna().astype(str).unique()[:20]
                    for i, s in enumerate(sig_examples, 1):
                        print(f"{i}. '{s}'")
        
        vprint(f"get_vasopressors returning {len(v)} records", v)
        return v
    except Exception as e:
        print(f"ERROR in get_vasopressors: {e}")
        if VERBOSE:
            traceback.print_exc()
        return pd.DataFrame()
...
def get_cultures(cdm, ancestor_df=None):
    try:
        ...
        if not po.empty and 'procedure_concept_id' in po.columns:
            ...
            cultures['person_id'] = pd.to_numeric(cultures['person_id'], errors='coerce').astype('Int64')
            cultures['visit_occurrence_id'] = pd.to_numeric(cultures['visit_occurrence_id'], errors='coerce').astype('Int64')
        ...
        vprint(f"get_cultures: found {len(result)} culture events", result)
        
        if VERBOSE and len(result) == 0:
            print("[VERBOSE] No cultures found. Checking available procedure concepts:")
            if not po.empty:
                print(po['procedure_concept_id'].value_counts().head(10))
        
        return result
    except Exception as e:
        print(f"ERROR in get_cultures: {e}")
        if VERBOSE:
            traceback.print_exc()
        return pd.DataFrame()
...
def get_antibiotics(cdm, ancestor_df=None):
    try:
        ...
        vprint(f"get_antibiotics: found {len(abx)} records for {len(abx_ids)} antibiotic concepts")
        ...
        if 'route_concept_id' in abx.columns:
            ...
            after = len(abx)
            if VERBOSE and before != after:
                vprint(f"get_antibiotics: filtered to systemic routes {before}->{after}")
        ...
        vprint(f"get_antibiotics returning {len(result)} records", result)
        
        if VERBOSE and len(result) == 0:
            print("[VERBOSE] No antibiotics found. Checking available drug concepts:")
            if not de.empty:
                print(de['drug_concept_id'].value_counts().head(10))
        
        return result
    except Exception as e:
        print(f"ERROR in get_antibiotics: {e}")
        if VERBOSE:
            traceback.print_exc()
        return pd.DataFrame()
