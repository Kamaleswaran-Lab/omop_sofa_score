...
# -------------------------------------------------------------------------
CONCEPT_SEEDS = {
    'platelets': [3007461, 3024929], # platelet count
    ...
    # vasopressor ingredients (RxNorm)
    ...
    'vasopressin': [11149],
    # broad antibiotic class - replace with full OHDSI set in production
    'antibiotics_systemic': [1738622, 1713332, 1717327, 1707164], # seed only
    'culture_procedure': [4046279], # microbial culture
    'mech_vent': [4052536, 4233974, 4302208],
    'rrt_procedure': [4146536, 4149391, 4048662]
}

# UCUM unit_concept_id mapping for conversion
UNIT_MAP = {
    'creatinine': {8840: 1.0, 8554: 1/88.4}, # mg/dL, umol/L
    'bilirubin': {8840: 1.0, 8554: 1/17.1},
    'pao2': {8645: 1.0, 8753: 7.50062}, # mmHg, kPa
    'platelets': {8847: 1.0, 8848: 1.0}, # 10^3/uL, 10^9/L (same numeric)
    'fio2': {8554: 0.01, 8713: 0.01, 0: 1.0} # percent to fraction, assume fraction if missing
}

def expand_concepts(ancestor_df, seed_ids):
    """Return all descendant concept_ids for seed list using concept_ancestor."""
...
def convert_units(df, value_col, unit_col, domain):
    """Apply unit conversion to SOFA standard units. Returns copy."""
    if df.empty or domain not in UNIT_MAP:
        return df
    df = df.copy()
    conv = df[unit_col].map(UNIT_MAP[domain]).fillna(1.0)
    # special handling for FiO2 percent
    if domain == 'fio2':
...
def get_measurements(cdm, seed_keys, domain=None, ancestor_df=None):
    """Extract measurements for one or more seed keys with unit conversion."""
    seeds = []
    for k in seed_keys:
...
def derive_map(cdm, ancestor_df=None):
    """Return MAP time series, preferring direct MAP then deriving from SBP/DBP."""
...
def derive_gcs(cdm, ancestor_df=None):
    """Return GCS total, using direct totals or summing components within 5 min."""
...
def get_paired_pao2_fio2(cdm, ancestor_df=None):
    """Return PaO2/FiO2 pairs within 60 minutes, with FiO2 normalized to fraction."""
...
def get_urine_output_24h(cdm, ancestor_df=None):
    """Aggregate urine output to 24h totals per calendar day."""
...
def get_vasopressors(cdm, ancestor_df=None):
    """Return vasopressor infusions with rate in mcg/kg/min where possible."""
    seeds = []
    ...
    # weight lookup for mcg/kg/min
    wt = get_measurements(cdm, [], ancestor_df=ancestor_df) # placeholder
    weight = cdm['measurement']
    weight = weight[weight['measurement_concept_id'].isin([3025315, 3013762])] # body weight
...
def get_cultures(cdm, ancestor_df=None):
    """Return culture times from procedure_occurrence and specimen."""
...
def get_antibiotics(cdm, ancestor_df=None):
    """Return systemic antibiotic exposures with start time."""
    abx_ids = expand_concepts(ancestor_df, CONCEPT_SEEDS['antibiotics_systemic'])
    de = cdm['drug_exposure']
    abx = de[de['drug_concept_id'].isin(abx_ids)].copy()
    # filter to systemic routes (IV, IM, PO) - example route concept_ids
    systemic_routes = [4128794, 4132161, 4136135] # IV, oral, IM - expand in production
