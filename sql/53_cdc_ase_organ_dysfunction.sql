...
lab_map AS (
  SELECT 
    l.*,
    CASE 
      WHEN measurement_concept_id IN (3022061,3016723) THEN 'creatinine' -- example OMOP
      WHEN measurement_concept_id IN (3022192) THEN 'egfr'
      WHEN measurement_concept_id IN (3024561) THEN 'bilirubin'
      WHEN measurement_concept_id IN (3013240) THEN 'platelet'
      WHEN measurement_concept_id IN (3024731) THEN 'lactate'
    END AS lab_type
  FROM labs l
),
...
renal AS (
  SELECT DISTINCT b.person_id, b.visit_occurrence_id, b.culture_date,
    MIN(l.lab_date) FILTER (WHERE l.lab_type='creatinine') AS event_date,
    'renal' AS od_type
  FROM baseline b
  ...
  GROUP BY b.person_id,b.visit_occurrence_id,b.culture_date,b.onset_type,b.win_start,b.win_end
  HAVING 
    ...
    OR
    MIN(CASE WHEN l.lab_type='egfr' THEN l.value_as_number END) <= 0.5 * MAX(CASE WHEN l.lab_type='egfr' THEN l.value_as_number END)
),
hepatic AS (
  SELECT DISTINCT b.person_id, b.visit_occurrence_id, b.culture_date,
