...
vaso_all AS (
  SELECT person_id, visit_occurrence_id, drug_date AS event_date, drug_concept_id FROM vaso_drug
  UNION ALL
  SELECT person_id, visit_occurrence_id, proc_date, procedure_concept_id FROM vaso_proc
),
vaso AS (
  SELECT DISTINCT pi.person_id, pi.visit_occurrence_id, pi.culture_date, va.event_date, 'vasopressor'::text AS od_type
  FROM pi JOIN vaso_all va USING (person_id, visit_occurrence_id)
  WHERE va.event_date BETWEEN pi.win_start AND pi.win_end
    AND NOT EXISTS (SELECT 1 FROM vaso_all va2 WHERE va2.person_id = va.person_id AND va2.visit_occurrence_id = va.visit_occurrence_id AND va2.concept_id = va.concept_id AND va2.event_date = va.event_date - 1)
),
vent_candidates AS (
...
vent AS (
  SELECT DISTINCT pi.person_id, pi.visit_occurrence_id, pi.culture_date, vc.proc_date AS event_date, 'ventilation'::text AS od_type
...
lab_candidates AS (
  SELECT m.person_id, m.visit_occurrence_id, DATE(m.measurement_date) AS lab_date, m.measurement_concept_id, m.value_as_number,
  ...
  FROM results_site_a.tmp_pi_labs m
),
renal AS (
  SELECT DISTINCT b.person_id, b.visit_occurrence_id, b.culture_date,
    MIN(CASE WHEN lc.lab_date BETWEEN b.win_start AND b.win_end THEN lc.lab_date END) AS event_date, 'renal'::text AS od_type
  FROM baseline b JOIN lab_candidates lc USING (person_id, visit_occurrence_id)
  WHERE lc.lab_type IN ('creatinine','egfr') AND lc.lab_date BETWEEN b.lookback_start AND b.win_end
  ...
  HAVING MAX(CASE WHEN lc.lab_date BETWEEN b.win_start AND b.win_end AND lc.lab_type='creatinine' THEN lc.value_as_number END) >= 2.0 *
         MIN(CASE WHEN lc.lab_date < b.win_start AND lc.lab_type='creatinine' THEN lc.value_as_number END)
     AND MIN(CASE WHEN lc.lab_date < b.win_start AND lc.lab_type='creatinine' THEN lc.value_as_number END) IS NOT NULL
),
hepatic AS (
  SELECT DISTINCT b.person_id, b.visit_occurrence_id, b.culture_date,
    MIN(CASE WHEN lc.lab_date BETWEEN b.win_start AND b.win_end THEN lc.lab_date END) AS event_date, 'hepatic'::text AS od_type
  FROM baseline b JOIN lab_candidates lc USING (person_id, visit_occurrence_id)
  WHERE lc.lab_type = 'bilirubin' AND lc.lab_date BETWEEN b.lookback_start AND b.win_end
  GROUP BY b.person_id, b.visit_occurrence_id, b.culture_date
  HAVING MAX(CASE WHEN lc.lab_date BETWEEN b.win_start AND b.win_end THEN lc.value_as_number END) >= 2.0
     AND MAX(CASE WHEN lc.lab_date BETWEEN b.win_start AND b.win_end THEN lc.value_as_number END) >= 2.0 *
         MIN(CASE WHEN lc.lab_date < b.win_start THEN lc.value_as_number END)
     AND MIN(CASE WHEN lc.lab_date < b.win_start THEN lc.value_as_number END) IS NOT NULL
),
platelet AS (
  SELECT DISTINCT b.person_id, b.visit_occurrence_id, b.culture_date,
    MIN(CASE WHEN lc.lab_date BETWEEN b.win_start AND b.win_end THEN lc.lab_date END) AS event_date, 'platelet'::text AS od_type
  FROM baseline b JOIN lab_candidates lc USING (person_id, visit_occurrence_id)
  WHERE lc.lab_type = 'platelet' AND lc.lab_date BETWEEN b.lookback_start AND b.win_end
  GROUP BY b.person_id, b.visit_occurrence_id, b.culture_date
...
lactate AS (
  SELECT DISTINCT b.person_id, b.visit_occurrence_id, b.culture_date, MIN(lc.lab_date) AS event_date, 'lactate'::text AS od_type
  FROM baseline b JOIN lab_candidates lc USING (person_id, visit_occurrence_id)
  WHERE lc.lab_type = 'lactate' AND lc.lab_date BETWEEN b.win_start AND b.win_end AND lc.value_as_number >= 2.0
  GROUP BY b.person_id, b.visit_occurrence_id, b.culture_date
)
