...
DROP VIEW IF EXISTS results_site_a.vw_neuro CASCADE;
...
SELECT 
    ...
    MAX(CASE WHEN m.measurement_concept_id IN (4093836, 3016335, 3009094, 3008223) 
        THEN m.value_as_number END) AS gcs_total,  -- 902,439 records
    MAX(CASE WHEN m.measurement_concept_id = 36684829 
        THEN m.value_as_number END) AS rass_score  -- 932,034 records
