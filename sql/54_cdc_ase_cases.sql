...
WITH pi AS (
  SELECT * FROM :results_schema.cdc_ase_presumed_infection
),
od AS (
...
final AS (
  SELECT 
    ...
    CASE WHEN (j.onset_date - vo.visit_start_date + 1) <= 2 THEN 'community-onset' ELSE 'hospital-onset' END AS onset_type,
    EXTRACT(YEAR FROM vo.visit_start_date) AS year
  FROM joined j
  JOIN :cdm_schema.visit_occurrence vo ON vo.visit_occurrence_id = j.visit_occurrence_id
  WHERE vo.visit_concept_id IN (9201,9203) -- inpatient, ERIP
)
SELECT * FROM final;
