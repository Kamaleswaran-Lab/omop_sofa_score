...
-- Event-scoped hourly SOFA component view.
--
-- The previous implementation generated a full min-to-max measurement grid per
-- person. This view only generates hours around infection-onset windows, which
-- keeps the row count tied to candidate episodes rather than lifetime EHR span.

DROP VIEW IF EXISTS :results_schema.vw_sofa_components CASCADE;

CREATE OR REPLACE VIEW :results_schema.vw_sofa_components AS
WITH infection_hours AS (
  SELECT DISTINCT
    io.person_id,
    gs.hr
...
raw_components AS (
  ...
  LEFT JOIN :results_schema.view_ventilation vent
    ON vent.person_id = b.person_id
   AND b.hr BETWEEN vent.start_datetime AND vent.end_datetime
  ...
  LEFT JOIN :results_schema.view_vasopressors_nee vp
    ON vp.person_id = b.person_id
   AND b.hr BETWEEN vp.start_datetime AND vp.end_datetime
  ...
  SELECT
    *,
    CASE
      ...
      WHEN pf_ratio >= 100 AND respiratory_support THEN 3
      WHEN pf_ratio < 100 AND respiratory_support THEN 4
      ELSE 2
    ...
    END AS liver_sofa,
    CASE
      WHEN max_vasopressor_nee_factor IS NOT NULL THEN 3
      WHEN map IS NULL THEN 0
      WHEN map >= 70 THEN 0
      ELSE 1
    END AS cardiovascular_sofa,
    CASE
      WHEN gcs_total IS NULL THEN 0
      WHEN gcs_total >= 15 THEN 0
      WHEN gcs_total >= 13 THEN 1
      WHEN gcs_total >= 10 THEN 2
      WHEN gcs_total >= 6 THEN 3
      ELSE 4
    END AS neurologic_sofa,
    CASE
      WHEN rrt_active THEN 4
      WHEN urine_24h_ml IS NOT NULL AND urine_24h_ml < 200 THEN 4
      WHEN urine_24h_ml IS NOT NULL AND urine_24h_ml < 500 THEN 3
      WHEN creatinine IS NULL THEN 0
    ...
    END AS renal_sofa,
    (
      (pf_ratio IS NOT NULL)::int +
      (platelets IS NOT NULL)::int +
      (bilirubin IS NOT NULL)::int +
      (map IS NOT NULL OR max_vasopressor_nee_factor IS NOT NULL)::int +
      (gcs_total IS NOT NULL)::int +
      (creatinine IS NOT NULL OR urine_24h_ml IS NOT NULL OR rrt_active)::int
    ) AS components_observed
  FROM raw_components
)
SELECT
  person_id,
