...
culture_starts AS (
    SELECT person_id, visit_occurrence_id, culture_datetime AS culture_time
    FROM {{results_schema}}.ase_blood_cultures
    WHERE culture_datetime IS NOT NULL
),
infection_pairs AS (
    SELECT a.person_id, a.visit_occurrence_id, a.abx_start, c.culture_time,
...
infection_candidates AS (
    SELECT person_id, visit_occurrence_id,
           ...
           CASE WHEN abx_start <= culture_time THEN 'antibiotic_first' ELSE 'culture_first' END AS infection_type,
           abx_start AS antibiotic_start,
           culture_time AS culture_start,
           hours_apart
    FROM infection_pairs
)
SELECT person_id, visit_occurrence_id, infection_onset, infection_type, antibiotic_start, culture_start
FROM (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY person_id, visit_occurrence_id ORDER BY infection_onset ASC, hours_apart ASC) AS rn
